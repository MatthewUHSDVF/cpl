angular.module('app').controller('GlobalController', function ($scope, $state, callService, RootCallDataService, $modal, $log) {
    holly.softphonebar = {
...
//        var dialoutTimeLength = holly.softphonebar._softphonebar_getDialoutTime(obj.DialoutTimeLength);
        ...
        _softphonebar_evtRing: function (event, callObject) {
            ...
            if (!hollyglobal.isMonitorPage && hollyglobal.ringEvent && typeof hollyglobal.ringEvent == 'function') {
                hollyglobal.ringEvent(callObject, function (params) {
                    //TODO ring 是否外呼
                    params.loginUser = hollyglobal.loginUser.split('@')[0];
                    params.FromCid = $scope.callPhoneNum || params.FromCid;
                    params.isDialout =  $scope.isDialout || 0;
                    RootCallDataService.setterPeerData(params);
                    $scope.isDialout = 0;
                    callService.callPhone(params.FromCid).success(function (data) {
                        console.dir(data);
                        RootCallDataService.setterRingData(data);
                        // $state.go('app.callScreenInfo.callScreen');
                        if($scope.isShowModal){
                            $scope.showDetailModal('lg');
                        }
                    }).error(function (ajax) {
                        console.log(ajax);
                    });
        ...
        _softphonebar_evtTalking: function (callObject) {
            if (!hollyglobal.isMonitorPage && hollyglobal.talkingEvent && typeof hollyglobal.talkingEvent == 'function') {
                hollyglobal.talkingEvent(callObject, function (peer) { //TODO talking
                    console.info(peer);
                    peer.isReceivedCall = 1;
                    RootCallDataService.setterTalkingData(peer);
                });
            }
        },
        _softphonebar_evtHangup: function (event, callObject) {
            if (!hollyglobal.isMonitorPage && hollyglobal.hangupEvent && typeof hollyglobal.hangupEvent == 'function') {
                hollyglobal.hangupEvent(callObject, function (peer) {  //TODO hangup
                    RootCallDataService.setterHangupData(peer);
                })
            }
        },
...
//        UserID: "82dc2510-bef4-11e3-9327-9b83bd1d8356"
    ...
    $scope.softphonebarDialout = holly.softphonebar.softphonebar_dialout;
    ...
    $scope.showDetailModal = function(size) {
        ...
        var modalInstance = $modal.open({
            templateUrl: 'pukang-callportal/callscreen/callScreen.html',
            controller: 'CallScreenController',
            size: size,
            backdrop:'static',
            keyboard:false
        });
        $scope.app.settings.header.headerFixedTop = true;
        modalInstance.result.then(function () {
            $scope.isShowModal = true;
            $scope.app.settings.header.headerFixedTop = false;
            RootCallDataService.setterRingData(undefined);
            RootCallDataService.setterTalkingData(undefined);
            RootCallDataService.setterHangupData(undefined);
            RootCallDataService.setterPeerData(undefined);
        },function () {
